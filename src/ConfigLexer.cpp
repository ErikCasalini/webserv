...
ConfigLexer::ConfigLexer(const string& filename)
	: m_filename(filename)
{};
...
// Read main config file, remove comments,
// expand includes and tokenize.
std::list<string> ConfigLexer::lex()
{
	std::stringstream config;
	_config_lexer::append_file_to_sstream(m_filename, config);

	string clean = _config_lexer::remove_comments_nl(config);
	if (clean.length() == 0)

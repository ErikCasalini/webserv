...
int socket_ex(int domain, int type, int protocol)
{
	...
	if (fd == -1)
		throw CriticalException(std::string("socket(): ") + strerror(errno));
	return (fd);
}
...
int	bind_ex(int sockfd, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (ret < 0)
		throw CriticalException(std::string("bind(): ") + strerror(errno));
	return (ret);
}
...
int listen_ex(int sockfd, int backlog)
{
	...
	if (ret < 0)
		throw CriticalException(std::string("listen(): ") + strerror(errno));
	return (ret);
}
...
int epoll_create_ex(int size)
{
	...
	if (ret < 0)
		throw CriticalException(std::string("epoll_create(): ") + strerror(errno));
	return (ret);
}
...
int epoll_ctl_ex(int epfd, int op, int fd, struct epoll_event *event)
{
	...
	if (ret < 0)
		throw CriticalException(std::string("epoll_ctl(): ") + strerror(errno));
	return (ret);
}
...
int epoll_wait_ex(int epfd, struct epoll_event *events, int maxevents, int timeout)
{
	...
	if (errno != 0 && errno != EINTR)
		throw CriticalException(std::string("epoll_wait(): ") + strerror(errno));
	return (ret);
}
